...
vtkfile_p = fe.File('results/mini_staggered_fluidpressure.pvd')
vtkfile_u = fe.File('results/mini_staggered_displacement.pvd')
#vtkfile_s_eff = fe.File('mini_staggered_effective_stress.pvd')
#vtkfile_sv = fe.File('mini_staggered_hydrostatic_totalstress.pvd')
vtkfile_s_total = fe.File('results/mini_staggered_totalstress.pvd')
...
aH = ( vH*pH*betaFS + dT*k_mu*fe.dot(fe.grad(vH), fe.grad(pH)) )*fe.dx   # Euler implicit

LH = ( vH*p_*betaFS - vH*(ev_-ev_n) )*fe.dx # 
...
y_staggered=np.linspace(tol, Height-tol, Nx+1)
p_staggered=np.zeros((Nt, Nx+1))
points=[ ( 0.5*Length, y_) for y_ in y_staggered ]
...
for n in range(Nt):     # time steps
    ...
    #plt.plot(y_staggered, p_staggered[n,:], color=color_code)
